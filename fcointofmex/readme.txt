...
二、简单说明：
本程序是对冲程序，fcoin现货挂单，fmex合约吃单对冲，实现fmex和fcoin双解锁。需要准备现货资金和btc合约资金，合约通过杠杆资金与现货数量保持一致，强烈建议将现货资金转入杠杆账户（不一定要借币），这样就完全独立开来，不会与其他程序冲突。程序收益比较稳定，但收益率不高。程序有2个，1个是maker主程序，一个是daemon守护程序。将该压缩包解压放到/home/目录下，运行daemon程序既可自动运行。默认配置了一个程序，如果需要多开，可以编辑daemon中的数字，改成需要的个数。同时配置对应编号的config文件。注意，每个maker进程必须单独放在一个目录里，并且不能重名。
...
margin_enable=1, =1现货使用杠杆交易，=0现货普通交易。强烈建议使用杠杆交易，把资金转入杠杆账户即可，可以不用借币，这样就不会与其他程序冲突，可以与其他程序并存。
quantity=0.01，fcoin单次挂单数量，单位是btc数量，建议单次小量，分多次对冲
offset=0.1，多个档位挂单区分开来，0.1为步进，建议紧挨着，这样解锁量大
...
fee_rate=0.05，手续费差价，如fcoin挂单赚0.01%，fcoin吃单付手续费0.065%，则手续费差价是0.055%，这里就填0.055就行。因为吃单可能有滑点，假设平均滑点是0.02%，这里可以再加0.02，防止亏损。如手续费差价0.055，就填0.075
C=7, 挂单灵敏度。自由调整，调小点容易成交，但可能利润不大，调大点难成交，但可能利润大，需要根据经验来调整
S=20, 5分钟振幅系数，为应对极端行情
cancel_time=3600，单子最长的撤单时间，单位秒，对撤单失败的补充
[ADVANCED]
taker_filter_min，合约吃单深度过滤，防止没深度吃单滑点太大。单位是张
